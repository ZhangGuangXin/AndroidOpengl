...
public class ObjLoader {
	public static Vertices3 load(Context context, int resourceID) {
		InputStream in = null;
		try {
			...
			for (int i = 0, vi = 0; i < numFaces * 3; i++) {
				...
				if (numNormals > 0) {//Float.floatToRawIntBits(vertices[i]);
					int normalIdx = facesNormals[i] * 3;
					verts[vi++] = Float.floatToRawIntBits(normals[normalIdx]);//normals[normalIdx];
//					Log.i("", "---normal 1: "+verts[vi-1]);
					verts[vi++] =  Float.floatToRawIntBits(normals[normalIdx +1]);//normals[normalIdx + 1];
//					Log.i("", "---normal 2: "+verts[vi-1]);
					verts[vi++] =  Float.floatToRawIntBits(normals[normalIdx +2]);//normals[normalIdx + 2];
//					Log.i("", "---normal 3: "+verts[vi-1]);
				}
			}
			...
			throw new RuntimeException("couldn't load '" + resourceID + "'", ex);
			// ex.printStackTrace();
		} finally {
			if (in != null)
				try {
